...
function SlotEditPanel({ slot, override, booking, courtPrice, slotDurationHours, onSave, onCancelBooking, onClose }) {
              ...
              <>
                {!showCancelConfirm ? (
                  <button
                    ...
                    className="mt-2 w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-red-500/10 border border-red-400/40 text-red-600 text-xs font-bold hover:bg-red-500/20 transition-colors"
                  >
                    <Trash2Icon className="w-3.5 h-3.5" />
                    لغو رزرو
                  </button>
                ) : (
                  <div className="mt-2 rounded-xl bg-red-500/10 border border-red-400/40 p-3 space-y-2">
                    <p className="text-xs font-bold text-red-600 text-center">آیا از لغو این رزرو مطمئن هستید؟</p>
                    <p className="text-[10px] text-muted-foreground text-center">این رزرو از اپ کاربر هم حذف می‌شود و نوتیفیکیشن ارسال می‌شود.</p>
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        disabled={cancelling}
                        className="flex-1 h-8 rounded-lg bg-red-500 hover:bg-red-600 text-white text-xs font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
                      >
                        {cancelling ? (
...
export default function CourtSlotsModal({ open, onClose, court }) {
  ...
  const handleCancelBooking = async (bookingId) => {
    const { ok } = await apiClient.patch(`/club-panel/bookings/${bookingId}/cancel`);
    if (!ok) return toast.error("خطا در لغو رزرو");

    // Remove override and booking from local state
    const key = `${selectedDay}|${activeSlot.start}`;
    setOverridesMap(prev => {
      const next = { ...prev };
      delete next[key];
      return next;
